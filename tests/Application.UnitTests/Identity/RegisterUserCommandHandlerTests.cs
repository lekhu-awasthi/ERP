...
public class RegisterUserCommandHandlerTests
{
    ...
    public async Task Handle_creates_user_with_hashed_password_and_email_unverified_status()
    {
        ...
        var handler = new RegisterUserCommandHandler(db, new FakePasswordHasher());

        var result = await handler.Handle(
            new RegisterUserCommand("Jane Doe", "jane@example.com", "9800000000", "Password123"),
    ...
    public async Task Handle_throws_conflict_when_email_already_registered()
    {
        ...
        await Assert.ThrowsAsync<ConflictException>(() => handler.Handle(
            new RegisterUserCommand("Jane Two", "JANE@example.com", "9800000001", "Password456"),
            CancellationToken.None));
    }
}
