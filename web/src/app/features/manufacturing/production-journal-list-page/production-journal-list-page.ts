...
import { ProductionJournalListItem, ProductionJournalStatus } from '../../../core/manufacturing/manufacturing.models';
...
/** List-page chrome for Production Journal, the same shape every ApprovableTransaction list uses.
...
export class ProductionJournalListPage {
  ...
  protected readonly items = signal<ProductionJournalListItem[]>([]);
  ...
  protected readonly statuses: StatusFilter[] = ['All', 'Draft', 'Approved', 'Void'];
  ...
  private load(): void {
    ...
    this.manufacturingService
      .listProductionJournals(this.organizationId, status === 'All' ? undefined : status, this.page(), this.pageSize(), this.filter.options())
      .subscribe({
        next: (result) => {
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load production journals.');
        },
      });
  }
  /**
   ...
   */
  protected readonly sortOptions = documentSortOptions('Journal date');
