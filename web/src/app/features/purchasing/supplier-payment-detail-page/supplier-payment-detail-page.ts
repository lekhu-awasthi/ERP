...
/** Mirror of payment-detail-page (see that component's doc comment) -- Direction=Paid hardcoded,
 * Supplier picker instead of Customer, allocations against this Supplier's Approved purchase
 * bills instead of invoices. Same shared Payment aggregate/endpoint, different Direction and
 * TargetDocumentType (confirmed live: "Debit [Supplier/AP account] / Credit [cash bank account]
...
export class SupplierPaymentDetailPage {
  ...
  protected readonly approvedPurchaseBills = signal<PurchaseBill[]>([]);
  ...
  protected billLabel(purchaseBillId: string): string {
    const bill = this.approvedPurchaseBills().find((b) => b.id === purchaseBillId);
    return bill ? bill.code : '—';
  }

  protected onAllocationBillChange(key: number, event: Event): void {
  ...
  protected suggestAllocations(): void {
    if (!this.contactId() || this.amount() <= 0) {
      this.errorMessage.set('Select a Supplier and enter an Amount first.');
      return;
    }
    ...
    this.paymentsService.getDefaultAllocations(this.organizationId, this.contactId(), this.amount(), 'Paid').subscribe({
      next: (suggestions) => {
  ...
  protected previewGlPosting(): void {
    ...
    this.paymentsService.previewPaymentGlPosting(this.organizationId, this.accountId(), this.amount(), 'Paid').subscribe({
      next: (lines) => {
  ...
  protected saveDraft(): void {
    ...
    const allocations: PaymentAllocationInput[] = this.allocations()
      ...
      .map((a) => ({ targetDocumentType: 'PurchaseBill', targetDocumentId: a.targetDocumentId, amount: a.amount }));
    ...
    if (this.isNew()) {
      this.paymentsService.createPayment(this.organizationId, request).subscribe({
        next: (result) => {
          ...
          this.router.navigate(['/organizations', this.organizationId, 'purchasing', 'supplier-payments', result.id]);
        },
        error: (err: unknown) => {
          this.saving.set(false);
