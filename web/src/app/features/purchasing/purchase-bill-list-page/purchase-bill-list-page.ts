...
export class PurchaseBillListPage {
  ...
  protected readonly items = signal<PurchaseBill[]>([]);
  ...
  private load(): void {
    ...
    this.purchasingService
      .listPurchaseBills(this.organizationId, status === 'All' ? undefined : status, this.page(), this.pageSize(), this.filter.options())
      .subscribe({
        next: (result) => {
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load purchase bills.');
        },
      });
  }
  /**
   ...
   */
  protected readonly sortOptions = documentSortOptions('Bill date');
