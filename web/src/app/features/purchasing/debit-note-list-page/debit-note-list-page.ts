...
export class DebitNoteListPage {
  ...
  private load(): void {
    ...
    this.purchasingService
      .listDebitNotes(this.organizationId, status === 'All' ? undefined : status, this.page(), this.pageSize(), this.filter.options())
      .subscribe({
        next: (result) => {
   ...
   */
  protected readonly sortOptions = documentSortOptions('Debit note date');
