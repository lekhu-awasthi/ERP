...
import { DeliveryNote, DeliveryNoteStatus } from '../../../core/sales/sales.models';
...
/** Phase 58 -- the Delivery Note list, the Sales Order list's chrome exactly: status tabs, the list
 * chrome's search / date range / location / sort, and the Stage column's custom-status picker (the live
 * "Delivery Note Order Status" group). */
@Component({
  selector: 'app-delivery-note-list-page',
  imports: [RouterLink, PaginationControl, CustomStatusPicker, NepaliDatePipe, ListChrome, LocationName, StatusBanner],
  templateUrl: './delivery-note-list-page.html',
})
export class DeliveryNoteListPage {
  ...
  protected readonly items = signal<DeliveryNote[]>([]);
  ...
  constructor() {
    ...
    this.configurationService.listCustomStatuses(this.organizationId).subscribe({
      next: (all) => this.customStatusOptions.set(all.filter((c) => c.isActive && c.documentType === 'DeliveryNote')),
    });
  }
  ...
  private load(): void {
    ...
    this.salesService
      .listDeliveryNotes(this.organizationId, status === 'All' ? undefined : status, this.page(), this.pageSize(), this.filter.options())
      .subscribe({
        next: (result) => {
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load delivery notes.');
        },
      });
  }
  /**
   ...
   */
  protected readonly sortOptions = documentSortOptions('Delivery note date');
