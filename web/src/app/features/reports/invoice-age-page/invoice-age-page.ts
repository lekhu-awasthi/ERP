...
/**
 * Invoice Age -- confirmed live 2026-09-03. Every outstanding document with its own age, where
 ...
 */
@Component({
  selector: 'app-invoice-age-page',
  imports: [PaginationControl, AmountPipe, BsDateInput, NepaliDatePipe, ReportLocationFilter, StatusBanner],
  templateUrl: './invoice-age-page.html',
})
export class InvoiceAgePage {
  ...
  protected readonly ageableTypes = CUSTOMER_AGEABLE_TYPES;
  ...
  constructor() {
    this.contactsService
      .listAllContacts(this.organizationId, 'Customer')
  ...
  private runExport(full: boolean, page: number, pageSize: number): void {
    ...
    this.reports
      .exportDocumentAge(
        this.organizationId, 'invoice-age', this.fromDate(), this.asOfDate(),
        this.contactId() || null, this.selectedTypes(), full, page, pageSize, this.locationId())
      .subscribe({
        next: (blob) => {
          this.exporting.set(false);
          triggerBlobDownload(blob, `InvoiceAge_${this.fromDate()}_${this.asOfDate()}.xlsx`);
        },
        error: (err: unknown) => {
          this.exporting.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not export the Invoice Age.');
        },
      });
  }
  ...
  private load(): void {
    ...
    this.reports
      ...
      .subscribe({
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load the Invoice Age.');
        },
      });
  }
}
