...
/**
 * Supplier Payable Summary -- confirmed live 2026-09-03: filters Period and Contact Group; columns
 * Supplier, Contact Group, Closing Balance; a footer Total row.
 *
 ...
 */
@Component({
  selector: 'app-supplier-payable-summary-page',
  imports: [PaginationControl, AmountPipe, BsDateInput],
  templateUrl: './supplier-payable-summary-page.html',
})
export class SupplierPayableSummaryPage {
  ...
  private runExport(full: boolean, page: number, pageSize: number): void {
    ...
    this.reports
      .exportContactBalanceSummary(
        this.organizationId, 'supplier-payable-summary', this.fromDate(), this.toDate(),
        this.contactGroupId() || null, full, page, pageSize)
      .subscribe({
        next: (blob) => {
          this.exporting.set(false);
          triggerBlobDownload(blob, `SupplierPayableSummary_${this.fromDate()}_${this.toDate()}.xlsx`);
        },
        error: (err: unknown) => {
          this.exporting.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not export the Supplier Payable Summary.');
        },
      });
  }
  ...
  private load(): void {
    ...
    this.reports
      ...
      .subscribe({
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load the Supplier Payable Summary.');
        },
      });
  }
}
