...
/**
 * Purchase By Supplier (Monthly) -- confirmed live 2026-09-03.
 *
 ...
 * and a row Total. The measure is Net Purchase, not Total Amount -- proved against the live figures.
 *
 ...
 */
@Component({
  selector: 'app-purchase-by-supplier-monthly-page',
  imports: [PaginationControl, AmountPipe, ReportLocationFilter],
  templateUrl: './purchase-by-supplier-monthly-page.html',
})
export class PurchaseBySupplierMonthlyPage {
  ...
  protected readonly report = signal<TradeByContactMonthlyDto | null>(null);
  ...
  constructor() {
    this.contactsService.listContactGroups(this.organizationId)
      .subscribe({ next: (g) => this.contactGroups.set(g) });
    this.load();
  }
  ...
  private runExport(full: boolean, page: number, pageSize: number): void {
    ...
    this.reports
      .exportTradeByContactMonthly(this.organizationId, 'purchase-by-supplier-monthly', this.fiscalYear(), this.contactGroupId() || null, full, page, pageSize, this.locationId())
      .subscribe({
        next: (blob) => {
          this.exporting.set(false);
          triggerBlobDownload(blob, `PurchaseBySupplierMonthly_BS${this.fiscalYear()}-${this.fiscalYear() + 1}.xlsx`);
        },
        error: (err: unknown) => {
          this.exporting.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not export the Purchase By Supplier (Monthly).');
        },
      });
  }
  ...
  private load(): void {
    ...
    this.reports
      .getTradeByContactMonthly(this.organizationId, 'purchase-by-supplier-monthly', this.fiscalYear(), this.contactGroupId() || null, this.page(), this.pageSize(), this.locationId())
      .subscribe({
        next: (report) => {
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load the Purchase By Supplier (Monthly).');
        },
      });
  }
}
