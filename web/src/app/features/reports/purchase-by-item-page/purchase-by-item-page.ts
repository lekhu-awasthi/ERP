    ...
    this.reports
      ...
      .subscribe({
        next: (blob) => {
          ...
          triggerBlobDownload(blob, `PurchaseByItem_${this.fromDate()}_${this.toDate()}.xlsx`);
        },
        error: (err: unknown) => {
          this.exporting.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not export the Purchase By Item.');
        },
      });
  }
  ...
  private load(): void {
    ...
    this.reports
      ...
      .subscribe({
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load the Purchase By Item.');
        },
      });
  }
}
