...
export class PurchaseByItemMonthlyPage {
  ...
  private runExport(full: boolean, page: number, pageSize: number): void {
    ...
    this.reports
      .exportTradeByItemMonthly(this.organizationId, 'purchase-by-item-monthly', this.fiscalYear(), full, page, pageSize, this.locationId())
      .subscribe({
        next: (blob) => {
          this.exporting.set(false);
          triggerBlobDownload(blob, `PurchaseByItemMonthly_BS${this.fiscalYear()}-${this.fiscalYear() + 1}.xlsx`);
        },
        error: (err: unknown) => {
          this.exporting.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not export the Purchase By Item (Monthly).');
        },
      });
  }
  ...
  private load(): void {
    ...
    this.reports
      .getTradeByItemMonthly(this.organizationId, 'purchase-by-item-monthly', this.fiscalYear(), this.page(), this.pageSize(), this.locationId())
      .subscribe({
        next: (report) => {
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load the Purchase By Item (Monthly).');
        },
      });
  }
}
