...
export class PurchaseByItemMonthlyPage {
  ...
  private runExport(full: boolean, page: number, pageSize: number): void {
    ...
    this.reports
      .exportTradeByItemMonthly(this.organizationId, 'purchase-by-item-monthly', this.fiscalYear(), full, page, pageSize)
      .subscribe({
        next: (blob) => {
  ...
  private load(): void {
    ...
    this.reports
      .getTradeByItemMonthly(this.organizationId, 'purchase-by-item-monthly', this.fiscalYear(), this.page(), this.pageSize())
      .subscribe({
        next: (report) => {
