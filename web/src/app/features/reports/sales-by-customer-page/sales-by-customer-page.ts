...
/**
 * Sales By Customer -- confirmed live 2026-09-03: filters Period and Contact Group; columns
 * Contact, Contact Group, Amount, Discount, Net Sales, Vat Amount, Total Amount, with a
 * footer Total over all five money columns. Amount is gross, so Amount less Discount equals
 * Net Sales on every row -- an identity the live figures satisfy.
 *
 * Returns are folded in as negative values rather than listed as their own rows, which is why a
 * heavily-credited customer can read negative here.
 */
@Component({
    ...
    this.reports
      ...
      .subscribe({
        next: (blob) => {
          ...
          triggerBlobDownload(blob, `SalesByCustomer_${this.fromDate()}_${this.toDate()}.xlsx`);
        },
        error: (err: unknown) => {
          this.exporting.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not export the Sales By Customer.');
        },
      });
  }
  ...
  private load(): void {
    ...
    this.reports
      ...
      .subscribe({
        ...
        error: (err: unknown) => {
          this.loading.set(false);
          this.errorMessage.set(extractErrorMessage(err) ?? 'Could not load the Sales By Customer.');
        },
      });
  }
}
