...
export class SalesByItemMonthlyPage {
  ...
  private runExport(full: boolean, page: number, pageSize: number): void {
    ...
    this.reports
      .exportTradeByItemMonthly(this.organizationId, 'sales-by-item-monthly', this.fiscalYear(), full, page, pageSize, this.locationId())
      .subscribe({
        next: (blob) => {
  ...
  private load(): void {
    ...
    this.reports
      .getTradeByItemMonthly(this.organizationId, 'sales-by-item-monthly', this.fiscalYear(), this.page(), this.pageSize(), this.locationId())
      .subscribe({
        next: (report) => {
