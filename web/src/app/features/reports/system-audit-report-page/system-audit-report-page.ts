...
@Component({
  ...
  imports: [RouterLink, PaginationControl, DatePipe],
...
export class SystemAuditReportPage {
  ...
  /** SalesOrder returns null -- no Angular detail page exists for it (Phase 5's "SalesOrder
   * shipped backend-only" scope decision, never retrofitted). Payment resolves to one of two
   * existing routes depending on Direction. Copied from transaction-approval-queue-page.ts's own
   * detailRoute method (Phase 12). */
