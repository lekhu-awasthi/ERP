...
this.exchangeRate.set(1);
  ...
  protected approve(overrideNegativeCashBalanceWarning = false): void {
    ...
    this.accountingService.approveCashTransfer(this.organizationId, this.routeCashTransferId, overrideNegativeCashBalanceWarning).subscribe({
      next: () => {
...
Approve anyway?`)) {
  ...
  protected print(): void {
    ...
    this.printingService.printDocument(this.organizationId, 'CashTransfer', this.routeCashTransferId).subscribe({
      next: (blob) => {
      ...
      error: (err: unknown) => {
        this.printing.set(false);
        tab?.close();
        this.errorMessage.set(extractErrorMessage(err) ?? 'Could not print cash transfer. Please try again.');
      },
    });
  }
}
