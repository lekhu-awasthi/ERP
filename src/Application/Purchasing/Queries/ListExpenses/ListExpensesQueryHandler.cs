...
public sealed class ListExpensesQueryHandler(IAppDbContext db, ICurrentUserService currentUser)
    : IRequestHandler<ListExpensesQuery, PagedResult<Expense>>
{
    public async Task<PagedResult<Expense>> Handle(ListExpensesQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            query = query.Where(x => x.Code.Contains(term) || (x.SupplierInvoiceReference != null && x.SupplierInvoiceReference.Contains(term)));
        }
        ...
        IOrderedQueryable<Expense> Order(IQueryable<Expense> source) => request.Sort switch
        {
