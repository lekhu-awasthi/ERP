...
public sealed class ListJournalVouchersQueryHandler(IAppDbContext db, ICurrentUserService currentUser)
    : IRequestHandler<ListJournalVouchersQuery, PagedResult<JournalVoucher>>
{
    public async Task<PagedResult<JournalVoucher>> Handle(ListJournalVouchersQuery request, CancellationToken cancellationToken)
    {
        ...
        IOrderedQueryable<JournalVoucher> Order(IQueryable<JournalVoucher> source) => request.Sort switch
        {
