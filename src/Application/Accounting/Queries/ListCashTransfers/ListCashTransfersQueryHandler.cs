...
public sealed class ListCashTransfersQueryHandler(IAppDbContext db, ICurrentUserService currentUser)
    : IRequestHandler<ListCashTransfersQuery, PagedResult<CashTransfer>>
{
    public async Task<PagedResult<CashTransfer>> Handle(ListCashTransfersQuery request, CancellationToken cancellationToken)
    {
        ...
        IOrderedQueryable<CashTransfer> Order(IQueryable<CashTransfer> source) => request.Sort switch
        {
