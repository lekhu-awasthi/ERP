...
public sealed class ListQuotationsQueryHandler(IAppDbContext db, ICurrentUserService currentUser)
    : IRequestHandler<ListQuotationsQuery, PagedResult<Quotation>>
{
    public async Task<PagedResult<Quotation>> Handle(ListQuotationsQuery request, CancellationToken cancellationToken)
    {
        ...
        IOrderedQueryable<Quotation> Order(IQueryable<Quotation> source) => request.Sort switch
        {
