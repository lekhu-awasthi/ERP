...
public static class AccountingEndpoints
{
    ...
    private static void MapJournalVoucherEndpoints(RouteGroupBuilder group)
    {
        group.MapGet("/journal-vouchers", async (
            Guid organizationId, JournalVoucherStatus? status, int? page, int? pageSize, string? search, DateOnly? fromDate, DateOnly? toDate, Guid? locationId, string? sort, ISender sender, CancellationToken ct) =>
        {
            var result = await sender.Send(
                new ListJournalVouchersQuery(organizationId, status, page ?? 1, pageSize ?? PagingDefaults.DefaultPageSize, search, fromDate, toDate, locationId, sort), ct);
            return Results.Ok(result);
        });
        ...
        {
            var result = await sender.Send(
                new ApproveJournalVoucherCommand(
    ...
    private static void MapCashTransferEndpoints(RouteGroupBuilder group)
    {
        group.MapGet("/cash-transfers", async (
            Guid organizationId, CashTransferStatus? status, int? page, int? pageSize, string? search, DateOnly? fromDate, DateOnly? toDate, Guid? locationId, string? sort, ISender sender, CancellationToken ct) =>
        {
            var result = await sender.Send(
                new ListCashTransfersQuery(organizationId, status, page ?? 1, pageSize ?? PagingDefaults.DefaultPageSize, search, fromDate, toDate, locationId, sort), ct);
            return Results.Ok(result);
        });
        ...
        {
            var result = await sender.Send(
                new ApproveCashTransferCommand(
                    organizationId, id, request?.OverrideNegativeCashBalanceWarning ?? false),
                ct);
            return Results.Ok(result);
        });
