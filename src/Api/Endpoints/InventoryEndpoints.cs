...
public static class InventoryEndpoints
{
    ...
    private static void MapWarehouseTransferEndpoints(RouteGroupBuilder group)
    {
        group.MapGet("/warehouse-transfers", async (
            Guid organizationId, WarehouseTransferStatus? status, int? page, int? pageSize, string? search, DateOnly? fromDate, DateOnly? toDate, Guid? locationId, string? sort, ISender sender, CancellationToken ct) =>
        {
            var result = await sender.Send(
                new ListWarehouseTransfersQuery(organizationId, status, page ?? 1, pageSize ?? PagingDefaults.DefaultPageSize, search, fromDate, toDate, locationId, sort), ct);
            return Results.Ok(result);
        });
    ...
    private static void MapInventoryAdjustmentEndpoints(RouteGroupBuilder group)
    {
        group.MapGet("/inventory-adjustments", async (
            ...
            string? search, DateOnly? fromDate, DateOnly? toDate, Guid? locationId, string? sort, ISender sender, CancellationToken ct) =>
        {
            var result = await sender.Send(
                new ListInventoryAdjustmentsQuery(organizationId, status, page ?? 1, pageSize ?? PagingDefaults.DefaultPageSize, search, fromDate, toDate, locationId, sort), ct);
            return Results.Ok(result);
        });
