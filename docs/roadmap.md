...
**Deliberately out of scope, decided not forgotten:** multi-UOM × variants (`ProductSecondaryUnit` is catalog metadata, not a stock line — it is on the sweep guard's allow-list with that reason); bulk-importing variants through Phase 21a's `ProductImporter`; and the three browser passes still outstanding from 21b/21c/22.
